...
public class booking extends JFrame implements ActionListener
{
	...
	{ 
		...
		setLocation(200,200);
		setResizable(false);
		setLocationRelativeTo(null);       //to set frames location in center
		...
		paybut.setBackground(Color.cyan);
		ImageIcon lbimg = new ImageIcon("images/c.gif");
		lblbl = new JLabel (lbimg);
		lblbl.setBounds(10,10,10,10);
		ImageIcon mbimg = new ImageIcon("");
		mblbl = new JLabel (mbimg);
		ImageIcon rbimg = new ImageIcon("");
		...
		showbut.setBackground(Color.orange);
		...
		for (int i = 0; i < leftside1.length; i++)
		{
			...
			xleft = 100;                                  //Moving back to the left for the next row
			yleft += 30;                                //Moving down for the next row
		}

		xmid = 350;
		ymid = 400;                                    //Initial positions
		...
		for (int i = 0; i < midside1.length; i++)
		{
			...
			xmid = 350;                              //Moving back to the left for the next row
			ymid += 30;                              //Moving down for the next row
		}

		xright = 600;
		yright = 400;                                //Initial positions
		...
		for (int i = 0; i < rightside1.length; i++)
		{
			...
			xright = 600;                          //Moving back to the left for the next row
	...
	public void paint(Graphics graf)
	{
		...
		switch(timechoice)
		{
			...
			case 7:
				...
				break;
			case 9:
				for (int i = 0; i < leftside9.length; i++) //Looping through the leftside at 9 block - gives initial size
				{
					for (int j = 0; j < 4; j++) //Looping through each row fully
					{
						leftside9[i][j].display(graf); //Displaying the left side @ 9
						midside9[i][j].display(graf); //Displaying the mid side @ 9
						rightside9[i][j].display(graf); //Displaying the right side @ 9
					}
				}
				break;
			default:
	...
	{
		selectedblock = block.getSelectedItem(); //The values needed to book seats and check for availability taken from text boxes
		...
		if (ev.getSource() == submitbut)
		{
			...
			if (selectseat == 15)
				...
				if (selectedtime == "13:00")                    //If we are trying to book a ticket a 1 o'clock
				{
				timechoice = 1;
				repaint();    
				if (selectedblock == "Left Block")              // 1. If we are trying to book a seat in the left block
				{
				...
				if (selectedblock == "Middle Block")            //2. If we are trying to book a seat in the middle block
				{
				...
				if (selectedblock == "Right Block")           //3. If we are trying to book a seat in the right block
				{
			...
			if (selectedtime == "15:00")                        //for 3 o'clock
			{
				timechoice = 3;
				repaint();

				if (selectedblock == "Left Block")               //1.left block
				{
				...
				if (selectedblock == "Middle Block")                  //2.middle block
				{
				...
				if (selectedblock == "Right Block")                   //3.right block
				{
			...
			if (selectedtime == "17:00")                           //for 5 o'clock
			{
				timechoice = 5;
				repaint();
			...
			if (selectedtime == "19:00")                                  //for 7 o'clock
			{
				timechoice = 7;
				repaint();
			...
			if (selectedtime == "21:00")                                     //for 9 o'clock
			{
				timechoice = 9;
				repaint();
		...
		if (ev.getSource() == paybut) //Pay button to generate tickets
		{
  				...
  				try{
					...
					for (int i = 0; i < totaltickets; i++)                             //Looping through the number of tickets booked
					{
						...
						output.write("________________________________________________________________________");
